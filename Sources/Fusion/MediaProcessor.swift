...
struct MediaProcessor {
    ...
    func run() async -> Bool {
        ...
        if outputFormat == "mp4" {
            ...
            // Video + TÜM ses izleri ayrı ayrı map edilir
            var cmd: [String] = ["-y", "-i", inputURL.path, "-map", "0:v:0"]
            for i in 0..<audioStreams.count {
                cmd.append(contentsOf: ["-map", "0:a:\(i)"])
            }
            ...
            // Tüm ses izlerini dil koduyla ekle
            ...
            let (status, _) = runCommand(mp4box, args: box)
            try? fm.removeItem(at: tmpDir)
            return status == 0
        ...
        // ══════════════════════════════════════════════════════════════════════
                ...
                if !attStreams.isEmpty {
                    ...
                    for s in attStreams {
                        ...
                        if !fname.isEmpty {
                            ...
                            if let attr = try? fm.attributesOfItem(atPath: fpath),
            ...
            // Ses dil metadata'larını koru
            for (i, aStream) in audioStreams.enumerated() {
                let tags       = aStream["tags"] as? [String: Any] ?? [:]
                let lang       = (tags["language"] as? String) ?? (tags["LANGUAGE"] as? String) ?? "und"
                let mappedLang = lMap[lang] ?? lang
                cmd1.append(contentsOf: ["-metadata:s:a:\(i)", "language=\(mappedLang)"])
            }

            cmd1.append(contentsOf: [
                "-c:v", "copy", "-c:a", "copy",
                "-map_metadata", "-1",
                "-map_metadata:s:v", "0:s:v",
                "-map_metadata:s:a", "0:s:a",
            ...
            // Aşama 2: Bölümleri ve fontları ekle
            var cmd2: [String] = [
            ...
            cmd2.append(outFile)
            let (status, _) = runCommand(ffmpeg, args: cmd2)
            try? fm.removeItem(at: tmpDir)
            return status == 0
        }
    }
    ...
    private func fileHasContent(_ url: URL) -> Bool {
        guard let attr = try? FileManager.default.attributesOfItem(atPath: url.path) else { return false }
        return (attr[.size] as? Int64 ?? 0) > 0
    }
}
